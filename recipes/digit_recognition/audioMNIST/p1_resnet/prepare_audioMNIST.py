"""
Creates the train, validation and test data manifest files for digit
classification.
...
def prepare_audioMNIST(
    ...
    # Checks if this phase is already done (if so, skip it)
    if skip(save_json_train, save_json_valid, save_json_test):
        logger.info("Preparation completed in previous run, skipping.")
        return

    # Lists files and create manifest from list
    logger.info(
    ...
    # Random splits the signal list into train, valid, and test sets
...
def create_json(wav_list, json_file):
    ...
    for wav_file in wav_list:
        # Reads the signal (to retrieve duration in seconds)
        ...
        # Gets the digit_id from utterance-id
        digit_id = uttid.split("-")[0].split("_")[0]

        # Creates entry for this utterance
        json_dict[uttid] = {
            "wav": relative_path,
            "length": duration,
            "digit_id": digit_id,
