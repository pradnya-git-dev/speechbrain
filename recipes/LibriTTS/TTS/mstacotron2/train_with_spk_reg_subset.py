...
class Tacotron2Brain(sb.Brain):
    ...
    def on_fit_start(self):
        ...
        self.spk_emb_mel_spec_encoder = MelSpectrogramEncoder.from_hparams(
          source=self.hparams.spk_emb_model_for_loss,
          run_opts={"device": self.device},
          freeze_params=True
        )
    ...
    def _compute_loss(self, predictions, batch, stage):
        ...
        if self.hparams.compute_spk_emb_loss:
          ...
          target_spk_embs = self.spk_emb_mel_spec_encoder.encode_batch(target_mels, target_mel_lens)
          ...
          preds_spk_embs = self.spk_emb_mel_spec_encoder.encode_batch(pred_mels_postnet, pred_mel_lens)
