...
TB_LOG_DIR = "/content/speechbrain/recipes/LibriTTS/TTS/exp10_disentanglement/exp100_sandbox/inf_tensorboard"
...
tacotron2_ms = MSTacotron2.from_hparams(source="/content/drive/MyDrive/mstts_saved_models/TTS/exp9_random_speaker_sampling/exp92_two_stage/exp922_tacotron2_nopt_ecapa_frozen_z_sample_ldc_sub_b1_e300",
                                        hparams_file="/content/speechbrain/recipes/LibriTTS/TTS/exp10_disentanglement/exp100_sandbox/tacotron2_inf_hparams.yaml",
                                        run_opts={"device": DEVICE})
...
with open(COS_SIM_SCORES_FILE, "w+") as cs_f:
  ...
  for wav_file in wav_list:
    ...
    for i in range(-3, 4):
      ...
      tb_writer.add_audio(
        uttid + f"_{i}_synthesized.wav",
        waveform_ms[i].squeeze(1),
        sample_rate=EXP_AUDIO_SR
      )

      #import pdb; pdb.set_trace()

      mel_spec_fig = plot_spectrogram(mel_output_ms[i])
      tb_writer.add_figure(
        uttid + f"_{i}",
        mel_spec_fig
      )

      embs_list.append(z_spk_emb[i + 3].squeeze())
      embs_labels_list.append(f"z_spk_emb_{i}_" + uttid.split("_")[0])
    ...
    # embs_labels_list.append("random_z_spk_emb_" + uttid)
    """
    target_emb = spk_emb
