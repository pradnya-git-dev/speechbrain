...
# Setup visualization
TB_LOG_DIR = "/content/speechbrain/recipes/LibriTTS/TTS/exp9_random_speaker_sampling/exp94_offline_gmm/default_mean_offline_gmm_tensorboard"
...
for spk_emb_file in SPK_EMB_FILES:
  with open(spk_emb_file, "rb") as speaker_embeddings_file:
    ...
    for (spk_emb_label, spk_emb) in speaker_embeddings.items():
      ...
      # train_spk_emb_tensors.append(spk_emb) # For visualization
...
for spk_id in spk_embs_dict.keys():
  ...
  num_valid_embs = 5 if 5 < len(spk_embs_dict[spk_id]) else len(spk_embs_dict[spk_id])
