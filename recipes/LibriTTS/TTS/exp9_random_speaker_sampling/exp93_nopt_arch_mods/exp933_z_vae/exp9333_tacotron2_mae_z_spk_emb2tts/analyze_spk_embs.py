...
TB_LOG_DIR = "/content/speechbrain/recipes/LibriTTS/TTS/exp9_random_speaker_sampling/exp93_nopt_arch_mods/exp933_z_vae/exp9332_tacotron2_ecapa/inf_tensorboard_libritts_700_samples"
...
tacotron2_ms = MSTacotron2.from_hparams(source="/content/drive/MyDrive/mstts_saved_models/TTS/exp9_random_speaker_sampling/exp93/exp9332_tacotron2_ecapa_ldc_sub_e545",
                                        hparams_file="/content/speechbrain/recipes/LibriTTS/TTS/exp9_random_speaker_sampling/exp93_nopt_arch_mods/exp933_z_vae/exp9332_tacotron2_ecapa/tacotron2_inf_hparams.yaml",
                                        run_opts={"device": DEVICE})
...
vc_pre_spk_embs, vc_z_spk_embs, vc_post_spk_embs, vc_z_means, vc_spk_emb_recs = [], [], [], [], []
vc_pre_spk_embs_labels, vc_z_spk_embs_labels, vc_post_spk_embs_labels, vc_z_means_labels, vc_spk_emb_recs_labels = [], [], [], [], []

same_z_diff_phrases_spk_embs, same_z_diff_phrases_spk_embs_labels = [], []
same_z_diff_phrases_spk_emb_recs, same_z_diff_phrases_spk_emb_recs_labels = [], []
...
same_spk_diff_phrases_phonemes = []
for phrase in same_spk_diff_phrases:
  phrase_phoneme_list = g2p(phrase)
  phrase = " ".join(phrase_phoneme_list)
  phrase = "{" + phrase + "}"
  same_spk_diff_phrases_phonemes.append(phrase)
...
with open(COS_SIM_SCORES_FILE, "w+") as cs_f:
  ...
  for wav_file in wav_list:
    ...
    mel_output_ms, mel_length_ms, alignment_ms, spk_emb_recs, z_mean, z_spk_embs = tacotron2_ms.encode_text(original_text, spk_emb)
    ...
    vc_spk_emb_recs.append(spk_emb_recs.squeeze())
    vc_spk_emb_recs_labels.append(spk_id)
    ...
    """
    mel_outputs, mel_length_ms, alignment_ms, spk_emb_recs, z_mean, z_spk_embs = tacotron2_ms.encode_batch(same_spk_diff_phrases_phonemes, spk_emb)
    for i in range(mel_outputs.shape[0]):
      ...
      same_z_diff_phrases_spk_emb_recs.append(spk_emb_recs[i].squeeze())
      same_z_diff_phrases_spk_emb_recs_labels.append(spk_id + f"_phrase_{i}")
...
combined_vc_spk_emb_recs = torch.stack(vc_spk_emb_recs)
...
tb_writer.add_embedding(
  combined_vc_spk_emb_recs,
  metadata=vc_spk_emb_recs_labels,
  tag="vc_spk_emb_recs"
)
...
combined_same_z_diff_phrases_spk_emb_recs = torch.stack(same_z_diff_phrases_spk_emb_recs)
...
tb_writer.add_embedding(
  combined_same_z_diff_phrases_spk_emb_recs,
  metadata=same_z_diff_phrases_spk_emb_recs_labels,
  tag="same_z_diff_phrases_spk_emb_recs"
)
