...
class FastSpeech2Brain(sb.Brain):
    ...
    def compute_objectives(self, predictions, batch, stage):
        ...
        if stage == sb.Stage.TRAIN:
          ...
          logger.info(f"KL step: {self.kl_beta_step}, KL beta: {kl_beta}")
          self.kl_beta_step += 1
        ...
        loss = self.hparams.criterion(predictions, y, spk_emb_triplets, kl_beta)
        kl_loss = loss["kl_loss"]
        logger.info(f"KL beta: {kl_beta}, KL loss: {kl_loss}")
