...
class Loss(nn.Module):
    ...
    def forward(self, predictions, targets, spk_emb_triplets, post_spk_embs, spk_ids):
        ...
        # Triplet loss calculation for speaker embeddings cosidering taregts as anchor
        anchor_spk_embs, pos_spk_embs, neg_spk_embs = spk_emb_triplets

        if anchor_spk_embs != None:
          spk_emb_loss_target = self.spk_emb_triplet_loss(anchor_spk_embs, pos_spk_embs, neg_spk_embs)
        else:
          spk_emb_loss_target = torch.Tensor([0]).to(mel_loss.device)

        # Triplet loss calculation without considering targets
        spk_emb_loss_preds = self.get_spk_emb_triplet_loss(spk_ids, post_spk_embs)
        ...
        total_loss = mel_loss*self.mel_loss_weight \
                    ...
                    + spk_emb_loss_target*self.se_triplet_loss_weight \
                    + spk_emb_loss_preds*self.se_triplet_loss_weight \
        ...
        loss = {
            ...
            "spk_emb_loss_target": spk_emb_loss_target*self.se_triplet_loss_weight,
            "spk_emb_loss_preds": spk_emb_loss_preds*self.se_triplet_loss_weight,
