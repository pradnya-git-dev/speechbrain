...
class Tacotron2Brain(sb.Brain):
    ...
    def on_fit_start(self):
        ...
        self.vocoder = HIFIGAN.from_hparams(
            source="/home/pradnya/projects/def-ravanelm/pradnya/saved_models/tts-hifigan-libritts-16kHz",
            savedir="tmpdir_vocoder",
            run_opts={"device": self.device},
        )

        self.frozen_spk_emb_mel_spec_encoder = MelSpectrogramEncoder.from_hparams(
    ...
    def compute_forward(self, batch, stage):
        ...
        _, input_lengths, target_mels, _, _ = inputs
    ...
    def _compute_loss(self, predictions, batch, stage):
        ...
        self.frozen_spk_emb_mel_spec_encoder.eval()

        target_mels = targets[0].detach().clone()
        pred_mels_postnet = predictions[1].detach().clone()

        target_spk_embs = self.frozen_spk_emb_mel_spec_encoder.encode_batch(target_mels)
        target_spk_embs = target_spk_embs.squeeze()
        target_spk_embs = target_spk_embs.to(self.device, non_blocking=True).float()

        preds_spk_embs = self.frozen_spk_emb_mel_spec_encoder.encode_batch(pred_mels_postnet)
    ...
    def run_inference_sample(self, stage):
        ...
        with torch.no_grad():
          ...
          if self.last_batch is None:
              return
          inputs, targets, _, labels, wavs, spk_ids = self.last_batch
          text_padded, input_lengths, target_mels, _, _ = inputs
