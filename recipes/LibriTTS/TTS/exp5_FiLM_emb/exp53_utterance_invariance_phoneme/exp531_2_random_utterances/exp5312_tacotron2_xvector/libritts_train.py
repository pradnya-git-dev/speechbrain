...
class Tacotron2Brain(sb.Brain):
    ...
    def on_fit_start(self):
        ...
        self.vocoder = HIFIGAN.from_hparams(source="speechbrain/tts-hifigan-libritts-16kHz", savedir="tmpdir_vocoder", run_opts={"device": self.device})
    ...
    def _compute_loss(self, predictions, batch, stage):
        ...
        loss: torch.Tensor
            the loss value
        """
        inputs, targets, num_items, labels, wavs, spk_embs, utt_inv_loss_pairs = batch
        text_padded, input_lengths, _, max_len, output_lengths = inputs
        loss_stats = self.hparams.criterion(
            predictions, targets, input_lengths, output_lengths, utt_inv_loss_pairs, self.last_epoch
        )
            ...
            raw_batch=self.hparams.progress_sample_logger.get_batch_sample(
                {
                    ...
                    "wavs": wavs,
                    "spk_embs": spk_embs,
                    "utt_inv_loss_pairs": utt_inv_loss_pairs
                }
            ),
        )
    ...
    def batch_to_device(self, batch):
        ...
        utt_inv_loss_pairs = utt_inv_loss_pairs.to(self.device, non_blocking=True).long()
        return (x, y, len_x, labels, wavs, spk_embs, utt_inv_loss_pairs)
    ...
    def on_stage_end(self, stage, stage_loss, epoch):
        ...
        if stage == sb.Stage.TRAIN and (self.hparams.epoch_counter.current % 10 == 0):
          ...
          _, targets, _, labels, wavs, spk_embs, utt_inv_loss_pairs = self.last_batch
    ...
    def run_inference_sample(self, stage):
        ...
        if self.last_batch is None:
            return
        inputs, targets, _, labels, wavs, spk_embs, utt_inv_loss_pairs = self.last_batch
    ...
    try:
      ...
      for dataset in hparams["splits"]:
          datasets[dataset] = sb.dataio.dataset.DynamicItemDataset.from_json(
              ...
              output_keys=["mel_text_pair", "wav", "label", "spk_id"],
          )
    except Exception as ex:
      print("EXCEPTION: ", ex)
...
if __name__ == "__main__":
    ...
    from compute_xvector_embeddings import compute_speaker_embeddings
    sb.utils.distributed.run_on_main(
