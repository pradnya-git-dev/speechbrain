...
class Tacotron2Brain(sb.Brain):
    ...
    def _compute_loss(self, predictions, batch, stage):
        ...
        pred_mel_out, pred_mel_out_postnet, pred_gate_out, pred_alignments = predictions
        pred_wav_tensors = self.vocoder.decode_batch(pred_mel_out_postnet)
        pred_wav_tensors = pred_wav_tensors.squeeze()

        pred_spk_embs = self.spk_emb_encoder.encode_batch(pred_wav_tensors, normalize=True)
        pred_spk_embs = pred_spk_embs.to(self.device, non_blocking=True).float()
        pred_spk_embs = pred_spk_embs.squeeze()

        predictions = pred_mel_out, pred_mel_out_postnet, pred_gate_out, pred_alignments, pred_spk_embs

        loss_stats = self.hparams.criterion(
            predictions, targets, input_lengths, output_lengths, target_spk_embs, self.last_epoch
        )
