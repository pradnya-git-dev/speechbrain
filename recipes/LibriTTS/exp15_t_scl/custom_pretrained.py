...
class EncoderClassifier(Pretrained):
    ...
    def encode_batch_mel_specs(self, mel_specs, wav_lens=None, normalize=False):
        """Encodes the input audio into a single vector embedding.
        ...
        # Assign full length if wav_lens is not assigned
        if wav_lens is None:
            wav_lens = torch.ones(mel_specs.shape[0], device=self.device)

        # Storing waveform in the specified device
        mel_specs, wav_lens = mel_specs.to(self.device), wav_lens.to(self.device)
        mel_specs = mel_specs.float()

        # Computing features and embeddings
        mel_specs = torch.transpose(mel_specs, 1, 2)
        feats = self.mods.mean_var_norm(mel_specs, wav_lens)
        embeddings = self.mods.embedding_model(feats, wav_lens)
        if normalize:
            embeddings = self.hparams.mean_var_norm_emb(
                embeddings, torch.ones(embeddings.shape[0], device=self.device)
