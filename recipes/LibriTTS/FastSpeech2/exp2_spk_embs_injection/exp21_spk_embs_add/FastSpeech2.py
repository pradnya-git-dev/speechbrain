...
class FastSpeech2(nn.Module):
    ...
    def __init__(
        ...
        self.spk_emb_mapper = LinearNorm(spk_emb_size, enc_d_model)
    ...
    def inject_spk_embs(self, hidden_states, spk_embs):
      ...
      if self.spk_emb_inject_method == "add":
        # import pdb; pdb.set_trace()

        spk_embs = self.spk_emb_mapper(spk_embs)
        spk_embs = torch.unsqueeze(spk_embs, 1).repeat(1, hidden_states.shape[1], 1)
        hidden_states = hidden_states + spk_embs
        return hidden_states
